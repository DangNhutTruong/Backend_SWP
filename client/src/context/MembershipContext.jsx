...
export const MembershipProvider = ({ children }) => {
  ...
  const upgradeMembership = async (targetMembership) => {
    ...
    if (!membershipTiers[targetMembership])
      return { success: false, error: "Gói thành viên không hợp lệ" };

    // Kiểm tra xem có đang nâng cấp không
    const currentTierIndex = Object.keys(membershipTiers).indexOf(
      user.membership || "free"
    );
    const targetTierIndex =
      Object.keys(membershipTiers).indexOf(targetMembership);
    ...
    try {
      ...
      if (result.success) {
        ...
        const transactions = JSON.parse(
          localStorage.getItem("membership_transactions") || "[]"
        );
        transactions.push({
      ...
      return {
        success: false,
        error: "Có lỗi xảy ra khi nâng cấp gói thành viên",
      };
    }
  };
  ...
  const checkFeatureAccess = (requiredMembership) => {
    ...
    if (
      !requiredMembership ||
      !["free", "premium", "pro"].includes(requiredMembership)
    ) {
    ...
    console.log(
      `Kiểm tra quyền truy cập: User ${user.email} có gói ${
        user.membership || "free"
      } (cấp ${userLevel})`
    );
    console.log(
      `Yêu cầu gói tối thiểu: ${requiredMembership} (cấp ${requiredLevel})`
    );
    console.log(
      `Kết quả: ${
