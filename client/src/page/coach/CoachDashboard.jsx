...
function CoachDashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Coach dashboard data from API:", result);

        // Transform API data
        const coachBookings = result.data.map((appointment) => ({
          id: appointment.id,
          ...
          coachName: appointment.coach_name || user.name,
        }));
        ...
        console.error("Failed to fetch coach dashboard data");
        ...
        const coachBookings = allAppointments.filter((appointment) => {
          return (
            appointment.coachName === user.fullName ||
            appointment.coachName === user.name
          );
        });
        ...
        setRecentBookings(coachBookings.slice(0, 5));
      }
    } catch (error) {
  ...
  const loadCoachAppointments = async () => {
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        const coachAppointments = result.data.map((appointment) => ({
          id: appointment.id,
          userId: appointment.user_id,
          userName: appointment.user_name,
          userEmail: appointment.user_email,
        ...
        const coachAppointments = storedAppointments.filter(
          (app) =>
            app &&
            app.coachName &&
            (app.coachName === user?.fullName || app.coachName === user?.name)
        );
        setAppointments(coachAppointments);
      }
    } catch (error) {
      ...
      coachAppointments.forEach((appointment) => {
        if (appointment && appointment.id) {
          ...
          const unreadCount = messages.filter(
            (msg) => msg && msg.sender === "user" && !msg.readByCoach
          ).length;
  ...
  const sendQuickMessage = (appointmentId, template) => {
    const quickMessages = {
      ...
      greeting:
        "Xin chào! Bạn có khỏe không? Hôm nay tình hình của bạn thế nào?",
      checkIn:
        "Bạn đã bao nhiều ngày không hút thuốc rồi? Hãy chia sẻ với tôi nhé!",
      encouragement: "Bạn làm rất tốt! Tôi rất tự hào về bạn! 🌟",
      advice:
        "Hãy nhớ uống nhiều nước và tập thể dục nhẹ để giảm căng thẳng nhé!",
    };
    ...
    <div className="coach-dashboard-container">
      <div className="dashboard-header">
        ...
        <div className="tab-navigation">
          ...
          <button
            className="tab-button manage-bookings-btn"
            onClick={() => navigate("/coach/bookings")}
          >
            <FaCog /> Quản lý Booking
          </button>
        </div>
      </div>
                ...
                <div className="appointment-info">
                  ...
                  <div className="appointment-datetime">
                    <span>
                      ...
                      {appointment.time || "Chưa xác định"}
                    </span>
                  </div>
                </div>
            ...
            <div className="chat-container">
              ...
              <div className="quick-messages">
                ...
                <div className="quick-message-buttons">
                  <button
                    onClick={() =>
                      sendQuickMessage(selectedAppointment.id, "greeting")
                    }
                  >
                    Chào hỏi
                  </button>
                  <button
                    onClick={() =>
                      sendQuickMessage(selectedAppointment.id, "checkIn")
                    }
                  >
                    Kiểm tra tình hình
                  </button>
                  <button
                    onClick={() =>
                      sendQuickMessage(selectedAppointment.id, "encouragement")
                    }
                  >
                    Động viên
                  </button>
                  <button
                    onClick={() =>
                      sendQuickMessage(selectedAppointment.id, "advice")
                    }
                  >
                    Lời khuyên
                  </button>
