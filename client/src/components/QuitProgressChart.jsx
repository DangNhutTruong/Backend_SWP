...
const debugLog = (...args) => {
  if (import.meta.env.DEV) {
    console.log(...args); // Sửa lỗi vòng lặp vô hạn
  }
};

debugLog("📊 QuitProgressChart.jsx FILE LOADED");
...
const QuitProgressChart = ({
  ...
  debugLog("🚀 QuitProgressChart KHỞI TẠO với props:", {
    userPlan,
    actualProgress,
    timeFilter,
    height,
  ...
  // Load dữ liệu thực tế từ backend
  useEffect(() => {
    const loadRealTimeData = async () => {
      try {
        ...
        // Lấy planId từ userPlan hoặc localStorage
        let planId = userPlan?.id;
        if (!planId) {
        ...
        if (planId) {
          // Lấy dữ liệu từ API
          const days =
            timeFilter === "7 ngày" ? 7 : timeFilter === "30 ngày" ? 30 : 90;
          const backendData = await getChartData(planId, days);

          // Chuyển đổi format dữ liệu
          const convertedData = convertProgressForChart(backendData);
          setRealTimeData(convertedData);

          debugLog("✅ Loaded real-time data from backend:", convertedData);
        } else {
        ...
        // Luôn tắt loading, kể cả khi có lỗi
        setIsLoading(false);
      }
    };
  ...
  const filterDataByTime = (data, filter) => {
    ...
    debugLog(`FILTER DEBUG: Ngày hôm nay là ${todayStr}`);
    switch (filter) {
      case "7 ngày":
        daysToShow = 7;
        break;
    ...
    if (import.meta.env.DEV) {
      debugLog(
        `Lọc dữ liệu: Hiển thị ${daysToShow} ngày gần nhất, từ ${cutoffDate.toLocaleDateString()}`
      );
      // Log input data before filtering
      debugLog("FILTER DEBUG: Input data length:", data?.length);
      if (data?.length > 0) {
        debugLog("FILTER DEBUG: Input data first item:", data[0]);
        debugLog("FILTER DEBUG: Input data last item:", data[data.length - 1]);
      }
    }
    ...
    const filteredData = Array.isArray(data)
      ? data.filter((item) => {
          ...
          if (data.length <= 7) {
            // Nếu ít dữ liệu (người dùng mới bắt đầu)
            debugLog(
              `FILTER DEBUG: ✅ Giữ lại dữ liệu ${item.date} (người dùng mới bắt đầu)`
            );
            return true;
          }
          ...
          if (item.date === todayStr) {
            debugLog(
              `FILTER DEBUG: ✅ Giữ lại dữ liệu ngày hôm nay (${todayStr})`
            );
            return true;
          }
          ...
          if (daysDiff <= 3) {
            // Log only recent data (now showing 3 days for more context)
            debugLog(
              `FILTER DEBUG: Date ${item.date} - Keep: ${result}, Days diff: ${daysDiff}`
            );
          }
    ...
    debugLog(`Kết quả lọc: ${filteredData.length} mục dữ liệu`);
    if (filteredData.length > 0) {
      debugLog("FILTER DEBUG: Filtered data first item:", filteredData[0]);
      debugLog(
        "FILTER DEBUG: Filtered data last item:",
  ...
  useEffect(() => {
    debugLog(
      "QuitProgressChart - Updating chart with realTimeData:",
      realTimeData
    );
